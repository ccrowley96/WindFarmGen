...
function simulate() {
  ...
  let pusherType = document.getElementById("pusherType").value;
  ...
  let startLiveSimulation = () => {
    ...
    interval = setInterval(function () {
      ...
      pushEvents(simulation.tick(pusherType), pusherType);
    }, livePushFrequency);
  };
...
function pushEvents(events, pusherType = "tsi") {
  if (pusherType === "tsi") {
    ...
    packetNumber++;
    updateTwins(events, packetNumber);
  }
}
...
window.onload = function () {
  let timestampSpacing = Number(
    document.getElementById("timestampSpacing").value
  );
  ...
  let updateTimeEstimate = () => {
    ...
    document.getElementById(
      "pushFrequencyLabel"
    ).innerHTML = timeToSend.toFixed(2);
  };

  document
    .getElementById("pushFrequency")
  ...
  document
    .getElementById("timestampSpacing")
    .addEventListener("change", (event) => {
  ...
  // Live push frequency
  document
    .getElementById("livePushFrequency")
    .addEventListener("change", (event) => {
      // update label
      const label = (document.getElementById("livePushLabel").innerHTML =
        event.target.value);
    });
