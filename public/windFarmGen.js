class WindSimulation {
  ...
  generateTsmInstances(download = true) {
        ...
        wm.systems.forEach(function (sys) {
          sys.devices.forEach(function (d) {
            var instance = {
              ...
              typeId: "tTBD",
              description:
                d.windMillSystem.windMill.name +
                " " +
                d.windMillSystem.name +
                " " +
                d.name,
              instanceFields: {
                Name: d.name,
            ...
            Object.keys(d.properties).forEach(
              (prop) => (instance.instanceFields[prop] = d.properties[prop])
            );
    ...
    if (download) downloadText(JSON.stringify({ put: instancesArray }));
  }
...
class WindMillDevice {
  ...
  tick(timestamp) {
    ...
    this.seedValue += step;
    var description =
      this.windMillSystem.windMill.name +
      " " +
      this.windMillSystem.name +
      " " +
      this.name;
    ...
    var event = {
      ...
      Timestamp: timestamp,
      Group: this.windMillSystem.name,
      Windmill: this.windMillSystem.windMill.name,
      Location: this.windMillSystem.windMill.windFarm.name,
      Name: this.name,
      Id: this.id,
